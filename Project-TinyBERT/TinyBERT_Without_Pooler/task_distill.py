...
from transformer.modeling import TinyBertForSequenceClassification as TinyBertForSequenceClassification_forteacher
from transformer.modeling_without_pooler import TinyBertForSequenceClassification
# from transformer.modeling_without_bert_pooler import TinyBertForSequenceClassification
# from transformer.modeling_without_token_type_embeddings import TinyBertForSequenceClassification
...
def do_eval(model, task_name, eval_dataloader,
    ...
    logger.info("Eval結束，縂耗時{}s，batch_num为{}，平均耗时{}s".format(TIME,len(eval_dataloader),TIME/len(eval_dataloader)))
...
def main():
    ...
    if not args.do_eval:
        teacher_model = TinyBertForSequenceClassification_forteacher.from_pretrained(args.teacher_model, num_labels=num_labels)
    ...
    if args.do_eval:
        ...
        time_end = time.time()  # 记录结束时间
        time_sum = time_end - time_start  # 计算的时间差为程序的执行时间，单位为秒/s
        logger.info("Eval結束，縂耗時{}s".format(time_sum))
        logger.info("***** Eval results *****")
        for key in sorted(result.keys()):
            logger.info("  %s = %s", key, str(result[key]))
    else:
